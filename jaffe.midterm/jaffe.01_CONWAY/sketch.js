...
function draw() {
  ...
  for (var i = 0; i < howwide; i++) {
    for (var j = 0; j < howtall; j++) {
      ...
      var p0 = img[whichimage].get(i-2, j-2)[0]>threshold; // upper left
      var p1 = img[whichimage].get(i, j-2)[0]>threshold; // upper mid
      var p2 = img[whichimage].get(i+2, j-2)[0]>threshold; // upper right
      var p3 = img[whichimage].get(i-2, j)[0]>threshold; // left
      var p4 = img[whichimage].get(i, j)[0]>threshold; // center pixel
      var p5 = img[whichimage].get(i+2, j)[0]>threshold; // right
      var p6 = img[whichimage].get(i-2, j+2)[0]>threshold; // lower left
      var p7 = img[whichimage].get(i, j+2)[0]>threshold; // lower mid
      var p8 = img[whichimage].get(i+2, j+2)[0]>threshold; // lower right
      var p9 = img[whichimage].get(i+random(2,6), j+1)[0]>threshold; // lower right
      var p10 = img[whichimage].get(i+random(1,5), j+1)[0]>threshold; // lower mid
      var p11 = img[whichimage].get(i+random(2,5), j+1)[0]>threshold; // lower left
      var neighbors = p0+p1+p2+p3+p5+p6+p7+p8+p9+p10+p11; // how many neighbors are alive?
      var result;
